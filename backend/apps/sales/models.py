...
class Customer(models.Model):
    ...
    payment_terms = models.CharField(
        max_length=20, choices=PAYMENT_TERMS_CHOICES, default="cash"
    )
...
class CustomerProduct(models.Model):
    ...
    customer = models.ForeignKey(
        Customer, on_delete=models.CASCADE, related_name="product_agreements"
    )
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name="customer_agreements"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["customer", "product"], name="custprod_customer_product_idx"
            ),
            models.Index(
                fields=["is_active", "valid_from"],
                name="custprod_active_valid_from_idx",
            ),
        ]
...
class SalesOrder(models.Model):
    ...
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.PROTECT, related_name="sales_orders"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["status", "order_type"], name="so_status_order_type_idx"
            ),
    ...
    def save(self, *args, **kwargs):
        if not self.order_number:
            self.order_number = generate_reference_number(
...
class SalesOrderLine(models.Model):
    ...
    product = models.ForeignKey(
        Product, on_delete=models.PROTECT, related_name="sales_order_lines"
    )
    ...
    cogs_total = models.DecimalField(
        max_digits=14, decimal_places=2, null=True, blank=True
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["sales_order", "product"], name="sol_sales_order_product_idx"
            ),
        ]
...
class Delivery(models.Model):
    ...
    warehouse = models.ForeignKey(
        Warehouse, on_delete=models.PROTECT, related_name="deliveries"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.delivery_number:
            self.delivery_number = generate_reference_number(
                "DEL", Delivery, "delivery_number"
            )
        super().save(*args, **kwargs)
...
class DeliveryLine(models.Model):
    ...
    batch = models.ForeignKey(
        Batch, on_delete=models.PROTECT, related_name="delivery_lines"
    )
...
class Invoice(models.Model):
    ...
    sales_order = models.OneToOneField(
        SalesOrder, on_delete=models.PROTECT, related_name="invoice"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.invoice_number:
            self.invoice_number = generate_reference_number(
